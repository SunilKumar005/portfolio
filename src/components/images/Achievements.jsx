...
const Achievement = ({ imgSrc, title, description }) => {
  const [ref, inView] = useInView({
    triggerOnce: false, // Allow repeating the animation
    rootMargin: '-100px 0px', // Adjust the root margin as needed
  });

  const variants = {
    hidden: { opacity: 0, y: 50 },
    visible: { opacity: 1, y: 0, transition: { duration: 2, ease: "easeInOut" } }, // Adjust duration and easing as needed
  };
  ...
  return (
    <motion.div ref={ref} style={{ ...styles.achievement, ...(inView ? styles.visible : styles.hidden) }} variants={variants} initial="hidden" animate={inView ? 'visible' : 'hidden'}>
      <motion.img
        src={require(`./achievement${imgSrc}.jpg`)}
        alt={title}
        style={styles.image}
      />
...
const styles = {
  ...
  achievement: {
    width: '100%',
    height: '100vh', // Take up the whole screen vertically
    display: 'flex',
    flexDirection: 'column',
    alignItems: 'center',
